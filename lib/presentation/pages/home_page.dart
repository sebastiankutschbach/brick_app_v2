...
class HomePage extends StatelessWidget {
        ...
        listener: (context, state) {
          if (state is HomePageError) {
            if (state.failure is CredentialsMissingFailure) {
              _showMaterialBanner(context, 'errMsgMissingCredentials'.tr());
            } else if (state.failure is WrongCredentialsFailure) {
              _showMaterialBanner(context, 'errMsgWrongCredentials'.tr());
            }
          }
        },
  ...
  _errorBody(BuildContext context, HomePageError state) {
    final errMsg = state.failure is CredentialsMissingFailure
        ? const Text('errMsgMissingCredentials').tr()
        : state.failure is WrongCredentialsFailure
            ? const Text('errMsgWrongCredentials').tr()
            : Text('errorLoadingSetList'.tr(args: [state.failure.message]));
  ...
  _buildSetListTile(BuildContext context, SetList setList) {
    return ListTile(
      ...
      subtitle: const Text('noOfSets').plural(setList.numberSets),
    ...
    ScaffoldMessenger.of(context).showMaterialBanner(
      MaterialBanner(
        ...
        actions: [
          ...
          ElevatedButton(
            child: const Text('toSettingsPage').tr(),
            onPressed: () {
              ScaffoldMessenger.of(context).hideCurrentMaterialBanner();
